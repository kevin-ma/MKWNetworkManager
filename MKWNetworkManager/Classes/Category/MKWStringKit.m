...
@implementation NSString (mks_safe)
...
@implementation NSString (mks_url)
    ...
    if (baseURL) {
        ...
        if (parameters.count > 0) {
            ...
            for (id key in sortedKeys) {
                [queryString appendFormat:@"%@=%@&", [key description], [[parameters[key] description] mkw_urlencode]];
            }
...
{
    static NSString * const kmkwCharactersToLeaveUnescapedInQueryStringPairKey = @"[].";
    static NSString * const kmkwCharactersToBeEscapedInQueryString = @":/?&=;+!@#$()',*";
    
    return (__bridge_transfer  NSString *)CFURLCreateStringByAddingPercentEscapes(kCFAllocatorDefault, (__bridge CFStringRef)self, (__bridge CFStringRef)kmkwCharactersToLeaveUnescapedInQueryStringPairKey, (__bridge CFStringRef)kmkwCharactersToBeEscapedInQueryString, kCFStringEncodingUTF8);
}

@end

@implementation NSString (mkw_path)
...
+ (NSString *)mkw_pathForTemporaryFile
{
    ...
    NSString *tmpPath = [[NSString mkw_temporaryPath] stringByAppendingPathComponent:(__bridge NSString *)newUniqueIdString];
...
@implementation NSString (mkw_common)
